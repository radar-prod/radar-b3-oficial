...
# ========================
# ✅ FUNÇÃO CORRIGIDA: Verificar lacunas
# ========================
...
def sistema_principal():
    ...
    if 'plano' not in st.session_state:
        st.session_state.plano = "Bronze"
    if 'expira' not in st.session_state:
        st.session_state.expira = datetime.now().date() + timedelta(days=15)
    ...
    # ============ MODO INTRADAY ============
        ...
        with col1:
            st.markdown("**Ações**")
            st.time_input("Abertura", key="abertura_acoes")
            st.time_input("Fechamento", key="fechamento_acoes")
        with col2:
            st.markdown("**Mini Índice / Mini Dólar**")
            st.time_input("Abertura", key="abertura_mini")
            st.time_input("Fechamento", key="fechamento_mini")

        # ✅ BOTÃO DE VALIDAÇÃO
        if st.button("🔍 Validar Arquivos (Verificar Lacunas)"):
            if not uploaded_files:
                st.warning("⚠️ Por favor, carregue pelo menos um arquivo antes de validar.")
            else:
        ...
        if data_min_global and data_max_global:
            ...
            if "configuracoes_salvas" in st.session_state:
                if st.button("🔍 Iniciar Rastreamento"):
                    if not uploaded_files:
                        st.warning("⚠️ Carregue os arquivos antes de iniciar o rastreamento.")
                    else:
